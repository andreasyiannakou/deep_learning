...
def draw_neural_net(ax, left, right, bottom, top, layer_sizes, coefs_, intercepts_):
    ...
    for n, (layer_size_a, layer_size_b) in enumerate(zip(layer_sizes[:-1], layer_sizes[1:])):
        ...
        for m in range(layer_size_a):
            for o in range(layer_size_b):
                ...
                #plt.text( xm1, ym1, str(round(coefs_[n][m, o],4)), rotation = rot_mo_deg, fontsize = 16)
    ...
    for n, (layer_size_a, layer_size_b) in enumerate(zip(layer_sizes[:-1], layer_sizes[1:])):
        ...
        for o in range(layer_size_b):
            ...
            #plt.text( xo1, yo1, str(round(intercepts_[n][o],4)), rotation = rot_bo_deg, fontsize = 16)    
...
# source: https://gist.github.com/craffel/2d727968c3aaebd10359 & https://gist.github.com/dvgodoy/0db802cfb8edd488dfbd524302ca4be7
...
coefs = np.array([[[1, 2], [-1, 3]], [[-1, -1], [1, 4]]])
intercepts = np.array([[0.5, -0.5], [-0.5]])
draw_neural_net(ax, .1, .9, .1, .9, layer_sizes, coefs, intercepts)
fig.savefig('nn_digaram_no_bias.png')
