...
def create_agent(memory, double_model, environment):
    ...
    agent = DQNAgent(states=environment.states, actions=environment.actions, network=network_spec, double_q_model=double_model, memory=memory,
        ...
        states_preprocessing=[dict(type='image_resize', width=84, height=84),
            dict(type='grayscale'),
...
network_spec = [dict(type='conv2d', size=32, window=8, stride=4), dict(type='conv2d', size=64, window=4, stride=2), dict(type='conv2d', size=64, window=3, stride=1), dict(type='flatten'), dict(type='dense', size=512)]

for memory_type in memory_types:
    #create filename
    fn = 'Space_Invaders_vis_' + str(model_type) + '_' + str(memory_type) + '.pkl'
    print(fn)
    d1 = datetime.datetime.now()
    # set the breakout atari environment
    environment = OpenAIGym('SpaceInvaders-v0', visualize=False)
